...
class Twitter(object):
    ...
    def postTweet(self, userId, tweetId):
        ...
        if userId not in self.tweets:
            ...
            self.tweets[userId].insert(0, (self.counter, tweetId))
        self.counter += 1
    ...
    def getNewsFeed(self, userId):
        ...
        if userId in self.follows.keys():
            feeds = [t for user in self.follows[userId] for t in self.tweets[user]]
            feeds.sort(key= lambda t: t[0], reverse= True)
            feeds = [c_f[1] for c_f in feeds]
            return feeds[:10]
        return []
    ...
    def unfollow(self, followerId, followeeId):
        ...
        :rtype: void
        """
        if followerId != followeeId and followerId in self.follows and followeeId in self.follows[followerId]:
